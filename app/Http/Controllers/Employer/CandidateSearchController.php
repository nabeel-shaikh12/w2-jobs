...
class CandidateController extends Controller
{
    ...
    public function index()
    {
        ...
        // }])
            ...
            ->paginate(12)
            ->withQueryString() // This should work correctly if using Laravel 8.x or later
    ...
    public function checkDownloadLimit()
    {
        ...
        // If the employer is not logged in or is not an employer, deny access
        if (!$employer || !$employer->isEmployer()) {
            return response()->json(['canDownload' => false, 'redirectUrl' => route('employer.memberships')], 403);
        }
    
        // Retrieve the current download count from the session
        $downloadCount = session()->get('resume_download_count_' . $employer->id, 0);
    
        // Check if the employer has reached the download limit
        if ($downloadCount >= 5) {
            // Return false to indicate the download limit has been reached and redirect to membership page
            return response()->json(['canDownload' => false, 'redirectUrl' => route('employer.memberships')]);
        }
    
        // Return true if the employer can still download resumes
