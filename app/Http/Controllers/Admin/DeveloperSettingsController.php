...
class DeveloperSettingsController extends Controller
{
    ...
    public function show($id)
    {
        ...
        if ($id == 'app-settings') {
            ...
            $textcode     = base64_decode(env('TxtAreaInp'));
            $appDebug     = env('APP_DEBUG');
            $timeZone     = env('TIME_ZONE', 'UTC');
            $default_lang = env('DEFAULT_LANG', 'en');

            return Inertia::render('Admin/Developer/App', compact('id', 'tzlist', 'languages', 'textcode', 'appName', 'appDebug', 'timeZone', 'default_lang', 'segments', 'buttons'));
        ...
        } elseif ($id == 'open-ai-settings') {
            $OPENAI_API_KEY = env('OPENAI_API_KEY');
            return Inertia::render('Admin/Developer/OpenAi', compact(
                'id',
                'OPENAI_API_KEY',
                'segments',
                'buttons'
            ));
        } elseif ($id == 'verification-settings') {

            $KYC_MUST_VERIFIED = env('KYC_MUST_VERIFIED');
            $EMAIL_MUST_VERIFIED = env('EMAIL_MUST_VERIFIED');

            return Inertia::render('Admin/Developer/Verification', compact(
                'id',
                'KYC_MUST_VERIFIED',
                'EMAIL_MUST_VERIFIED',
                'segments',
         ...
         elseif ($id == 'recaptcha-settings') {

            $GOOGLE_RECAPTCHA_KEY = env('GOOGLE_RECAPTCHA_KEY');
            $GOOGLE_RECAPTCHA_SECRET = env('GOOGLE_RECAPTCHA_SECRET');
            return Inertia::render('Admin/Developer/Recaptcha', compact(
                'id',
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($id == 'app-settings') {
            ...
            $encoded = base64_encode($request->TxtAreaInp);
            $this->editEnv('TxtAreaInp', $encoded);
            ...
            $this->editEnv('KYC_MUST_VERIFIED', filter_var($request->KYC_MUST_VERIFIED, FILTER_VALIDATE_BOOLEAN), true);
            $this->editEnv('EMAIL_MUST_VERIFIED', filter_var($request->EMAIL_MUST_VERIFIED, FILTER_VALIDATE_BOOLEAN), true);
         ...
         elseif ($id == 'recaptcha-settings') {
            $this->editEnv('GOOGLE_RECAPTCHA_KEY', $request->GOOGLE_RECAPTCHA_KEY);
            $this->editEnv('GOOGLE_RECAPTCHA_SECRET', $request->GOOGLE_RECAPTCHA_SECRET);
