...
const canDownloadResume = ref(true);
const downloadLimit = 28; 
const downloadResume = (resumeLink) => {
  let downloadCount = localStorage.getItem('resumeDownloadCount') || 0;

  if (downloadCount >= downloadLimit) {
    canDownloadResume.value = false;
    alert('You have reached the download limit of 5 resumes. Upgrade your membership to download more resumes.');
  } else {
    downloadCount++;
...
onMounted(() => {
  const downloadCount = localStorage.getItem('resumeDownloadCount') || 0;
...
<template>
  <div class="col-xxl-6 col-lg-6 col-sm-6 d-flex">
    <div
    ...
    >
      <div class="d-flex justify-content-between align-items-center">
        ...
        <!-- Conditionally show the resume download button if the user can still download resumes -->
        <div v-if="canDownloadResume && candidate.resume" class="ms-md-2">
          <a style="font-size: 12px;" @click.prevent="downloadResume(candidate.resume)" class="resume-btn tran3s">
            <i class="bi bi-eye" aria-hidden="true"></i> Show Resume
          </a>
        </div>
      </div>
      ...
      <h4 class="candidate-name mt-15 mb-0">
        <Link :href="route('candidates.show', candidate.username)" class="tran3s">{{ candidate.name }}</Link>
      </h4>
      <div class="candidate-post">{{ candidate.service?.title }}</div>

      <ul class="cadidate-skills style-none d-flex align-items-center justify-content-center justify-content-md-between pt-30 sm-pt-20 flex-wrap pb-10">
        <li class="text-nowrap" v-for="(item, index) in tags(candidate.tags).tagItems ?? []" :key="index">
          {{ item.title }}
        </li>
