...
<template>
  <Seo :metaData="seo" />
  <!-- <Breadcrumb :title="candidate.name" href="/" :href-text="trans('Home')" :subtitle="candidate.name" /> -->
          ...
          <div class="candidates-profile-details me-xxl-5 pe-xxl-4">
            ...
            <div class="inner-card border-style mb-75 lg-mb-50">
              ...
              <div class="time-line-data position-relative pt-15">
                <div class="info position-relative" v-for="(item, index) in candidate.educations ?? []" :key="index">
            ...
            <!-- /.inner-card -->
            <div class="inner-card border-style lg-mb-50 mb-60" v-if="meta.work_experiences?.length">
              <h3 class="title">{{ trans('Work Experience') }}</h3>
              <div class="time-line-data position-relative pt-15">
                <div v-for="(item, index) in meta.work_experiences" :key="index" class="info position-relative">
                  <div class="numb fw-500 rounded-circle d-flex align-items-center justify-content-center">
                    {{ index + 1 }}
                  </div>
            ...
            <template v-if="candidate.portfolios?.length">
              ...
              <div class="candidate-portfolio-slider">
                <div class="item" v-for="(item, index) in candidate.portfolios" :key="index">
                  <img v-lazy="item.preview" alt="preview" class="w-100" data-bs-toggle="tooltip"
            ...
            <div class="mt-30 position-relative" v-if="candidate.candidate_reviews?.length">
              ...
              <div class="candidate-review-slider">
                <div v-for="(review, index) in candidate.candidate_reviews" :key="index" class="item">
                  <div class="feedback-block-four">
                    ...
                    <div class="d-flex align-items-center">
                      <img v-lazy="review.company?.avatar == null
                          ? `https://ui-avatars.com/api/?name=${review.company?.name}`
                          : `${review.company?.avatar}`
                        " alt="img" class="author-img rounded-circle" />
                      <div class="ms-3">
          ...
          <div class="cadidate-profile-sidebar ms-xl-5 ms-xxl-0 md-mt-60">
            <div class="cadidate-bio bg-wrapper bg-color md-mb-40 mb-60">
              <div class="pt-25">
                <div class="cadidate-avatar m-auto">
                  <img v-lazy="candidate?.avatar == null
                      ...
                      : `${candidate?.avatar}`
                    " alt="img" class="lazy-img rounded-circle w-100" />
                </div>
              </div>
              ...
              <ul class="style-none">
                ...
                <li v-if="candidate_reviews_avg_ratting > 0">
                  ...
                  <div>
                    <div class="d-flex gap-1">
                      <span v-for="(item, index) in Math.round(
                ...
                <li v-if="candidate.hasSocial">
                  ...
                  <div>
                    <a v-if="meta?.social?.facebook" :href="meta?.social?.facebook ?? '#'" class="me-3"><i
                        class="bi bi-facebook"></i></a>
                    <a v-if="meta?.social?.instagram" :href="meta?.social?.instagram ?? '#'" class="me-3"><i
                        class="bi bi-instagram"></i></a>
                    <a v-if="meta?.social?.twitter" :href="meta?.social?.twitter ?? '#'" class="me-3"><i
                        class="bi bi-twitter"></i></a>
                    <a v-if="meta?.social?.linkedin" :href="meta?.social?.linkedin ?? '#'"><i
                        class="bi bi-linkedin"></i></a>
                  </div>
                </li>
              </ul>
              <a v-if="meta.custom_cv" :href="meta.custom_cv" download=""
                class="btn-ten fw-500 w-100 tran3s mt-15 text-center text-white">{{ trans('Download CV') }}</a>
            </div>
            <!-- /.cadidate-bio -->
            ...
            <div class="map-area md-mb-40 mb-60">
              <div class="gmap_canvas h-100 w-100">
                <iframe class="gmap_iframe h-100 w-100"
                  :src="`https://maps.google.com/maps?width=600&amp;height=400&amp;hl=en&amp;q=${candidate.state},${candidate.country}&amp;t=&amp;z=12&amp;ie=UTF8&amp;iwloc=B&amp;output=embed`"></iframe>
              </div>
            </div>
            ...
            <template v-if="authUser && authUser.role == 'employer'">
              ...
              <div v-else class="email-form bg-wrapper bg-color">
                ...
                <form @submit.prevent="submit">
                  ...
                  <div class="d-sm-flex mt-20">
                    <SpinnerBtn type="submit" :processing="form.processing"
              ...
              <a :href="candidate.resumeUrl"
                class="btn-ten fw-500 d-flex align-items-center justify-content-center w-100 tran3s text-center text-white"
                target="_blank">
                {{ trans('See Resume') }}
              </a>
            </template>
