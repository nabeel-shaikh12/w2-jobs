          ...
          <div class="card-body">
            <form @submit.prevent="createPost" class="space-y-3">
              ...
              <div>
                ...
                <input @input="previewFile" type="file" class="input" required name="preview" accept="image/*" />
              </div>
              <div>
                <label>{{ trans('Short Description') }}</label>
                <textarea v-model="form.short_description" name="short_description" required class="textarea"
              ...
              <div>
                ...
                <MultiSelect v-model="selectedCategories"  display="chip" :options="Categories" optionLabel="name"
                  class="w-full md:w-20rem selectstyle" />
              </div>
              <div class="mb-2">
                <label>{{ trans('Select Tags') }}</label>
                <MultiSelect v-model="selectedTags" display="chip" :options="Tags" optionLabel="name"
                  class="w-full md:w-20rem selectstyle" />
              </div>
              <div>
              ...
              <div>
                ...
                <input @input="metaFile" type="file" class="input" name="meta_image" required accept="image/*" />
              </div>
              <div>
                <label>{{ trans('SEO Meta Description') }}</label>
                <textarea v-model="form.meta_description" name="meta_description" required
                  class="input h-100"></textarea>
              </div>
              <div>
...
const Categories = ref(
  Object.entries(props.categories).map(([id, title]) => ({ name: title, code: id }))
);

const selectedTags = ref([]);
const Tags = ref(
  Object.entries(props.tags).map(([id, title]) => ({ name: title, code: id }))
);
...
const createPost = () => {
  form.categories = selectedCategories.value.map(item => item.code);
  form.tags = selectedTags.value.map(item => item.code);
